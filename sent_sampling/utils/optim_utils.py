...
from sent_sampling.utils.opt_exp_design import coordinate_ascent_eh, coordinate_ascent_parallel_eh, second_order_rdm, create_rdm, MI, coordinate_ascent
#import tools
...
def Distance_grp_JSD(S,group_act, distance='correlation'):
    '''ds_grp_jsd'''
    NotImplementedError
...
class optim:
    ...
    def gpu_object_function_ds_plus_jsd(self, S, debug=False):
        ...
        assert (XY_corr_sample_tensor.shape[1] > XY_corr_sample_tensor.shape[0])
        # do the same for pairs rand
        XY_corr_sample_rand = [XY_corr[pairs_rand[:, 0], pairs_rand[:, 1]].to(self.device) for XY_corr in
                               self.XY_corr_list]
        XY_corr_sample_tensor_rand = torch.stack(XY_corr_sample_rand).to(self.device)
        XY_corr_sample_tensor_rand = torch.transpose(XY_corr_sample_tensor_rand, 1, 0)
        if XY_corr_sample_tensor_rand.shape[1] < XY_corr_sample_tensor_rand.shape[0]:
...
N_s=[ 10,  25,  50,  75,  80, 100, 125, 150, 168, 170, 184, 186, 192, 194, 200,205, 210, 225, 250, 275, 300]
n_inits=[1,2]
run_gpu=[True,False]
low_dim=[True,False]
pca_var=[.9,.95]
