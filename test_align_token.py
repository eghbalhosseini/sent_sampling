...
# kk=0
# for context_id in enumerate(aligned_tokens):
#     print(context_id)
#     kk+=1
# len(candidate._model.tokenize(sentence_stimuli.sentence[0]))
#
# kk=0
# for context_id in enumerate(aligned_tokens_debug):
#     print(context_id)
#     kk+=1
...
#sentence_stimuli = StimulusSet({'sentence': ' '.join(part_stimuli['word']),reset_column: list(set(part_stimuli[reset_column]))})
...
#tokenized_sent=candidate._model._model_container.tokenizer.tokenize('this is test')
#sentence_stimuli.sentence.values
#aligned_tokens=candidate._model._model_container.align_tokens(tokenized_sentences=tokenized_sentences,additional_tokens=[],sentences=sentences,max_num_words=512,use_special_tokens=False)
