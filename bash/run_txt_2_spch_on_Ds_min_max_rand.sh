...
for model in tts_models/en/ek1/tacotron2  tts_models/en/ljspeech/tacotron2-DDC tts_models/en/ljspeech/tacotron2-DDC_ph \
...
if [ "$LINE_COUNT" -gt "$run_val" ]; then
  echo "running  ${LINE_COUNT} "
   #nohup /cm/shared/admin/bin/submit-many-jobs 1360 950 600 350 txt_2_spch.sh $GRAND_PIPE_FILE
#   nohup /cm/shared/admin/bin/submit-many-jobs 10 5 3 2 txt_2_spch.sh $GRAND_PIPE_FILE &
   nohup /cm/shared/admin/bin/submit-many-jobs $LINE_COUNT  250 300 50 txt_2_spch.sh $GRAND_PIPE_FILE
  else
    echo $LINE_COUNT
fi
