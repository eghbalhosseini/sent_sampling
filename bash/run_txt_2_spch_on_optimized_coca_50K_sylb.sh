...
while read string; do
      ...
      possible_file="/om/user/ehoseini/MyData/sent_sampling/coca_spok_filter_punct_50K_sylb/wav/coca_spok_filter_punct_50K_sylb_${framename}.wav"
      if [ -f "$possible_file" ]
      then
        true
      else
    ...
    i=$(expr ${i} + 1)
done < "/om/user/ehoseini/MyData/sent_sampling/coca_spok_filter_punct_50K_sylb/text/sentences_coca_spok_filter_punct_50K_sylb.txt"

echo $LINE_COUNT
run_val=0
if [ "$LINE_COUNT" -gt "$run_val" ]; then
  echo "running  ${LINE_COUNT} "
   nohup /cm/shared/admin/bin/submit-many-jobs $LINE_COUNT 750 900 150 txt_2_spch_coca_50K.sh $GRAND_PIPE_FILE
