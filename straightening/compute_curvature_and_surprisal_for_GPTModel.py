...
from transformers import AutoConfig, AutoModel, AutoModelWithLMHead,AutoTokenizer,GPTNeoXForCausalLM, AutoModelForSequenceClassification,AutoModelForCausalLM,AutoModelForTokenClassification, OPTConfig, OPTModel, GPT2Config,GPT2Model
...
if __name__ == '__main__':
    ...
    new_model = AutoModelForCausalLM.from_pretrained(newmodelname)

    new_model_untrained_config = GPT2Config.from_pretrained(newmodelname)
    new_model_untrained = GPT2Model(new_model_untrained_config)
    ...
    new_model.cuda()
    tokenized_new = [tokenizer.tokenize(x) for x in sentences]
    # get ids
    indexed_tokens_new=[tokenizer.convert_tokens_to_ids(x) for x in tokenized_new]
    ...
    for i, curv in tqdm(enumerate(curve_new)):
        ...
        curv_unt = curve_[i, :]
        nan_idx = np.logical_or(np.logical_or(np.isnan(curv), np.isnan(model_surp_avg)), np.isnan(curv_unt))
        curv = curv[~nan_idx]
        curv_unt = curv_unt[~nan_idx]
    ...
    # set y tick
    #ax.set_yticks(np.arange(-.1, .3, .05))
    # set y tick labels to go with step of .1 from -.1 to .2 with percision of 2
    #ax.set_yticklabels(['{:.2f}'.format(x) for x in np.arange(-.1, .3, .05)], fontsize=6)

    newmodelname_=newmodelname.replace('/','-')
    fig.savefig(os.path.join(ANALYZE_DIR, f'curvature_and_surpsial_{newmodelname_}.pdf'), transparent=True)
    fig.savefig(os.path.join(ANALYZE_DIR, f'curvature_and_surprisal_{newmodelname_}.png'), transparent=True)
