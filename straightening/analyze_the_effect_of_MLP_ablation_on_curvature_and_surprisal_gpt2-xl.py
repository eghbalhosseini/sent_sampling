...
if __name__ == '__main__':
    ...
    ablated_config.architectures = ["GPT2LMHeadModel-MLP_ablated"]
    ...
    for name, param_src in ablated_state_dict.items():
        if f"h.{layer_to_ablate}.mlp" in name:
    ...
    #a=ablated_model.state_dict()['transformer.h.45.mlp.c_fc.weight']
    #b=ablated_model.state_dict()['transformer.h.45.mlp.c_proj.weight']
    #torch.matmul(a,b)
    ...
    fig.savefig(os.path.join(ANALYZE_DIR, f'surprisal_{modelname}_layer_{layer_to_ablate}_MLP_ablated_long_sent_k_{k_size}.pdf'), transparent=True)
    fig.savefig(os.path.join(ANALYZE_DIR, f'surprisal_{modelname}_layer_{layer_to_ablate}_MLP_ablated_long_sent_k_{k_size}.png'), transparent=True)
