...
if __name__ == '__main__':
    ...
    ax.fill_between(np.arange(curve_new.shape[0]),
                    np.nanmean(curve_new, axis=1) * 180 / np.pi - np.nanstd(curve_new, axis=1) * 180 / np.pi,
                    np.nanmean(curve_new, axis=1) * 180 / np.pi + np.nanstd(curve_new, axis=1) * 180 / np.pi, alpha=.2,
                    color=(1, 0, 0))
    ...
    ax.plot(np.arange(curve_change.shape[0]), np.nanmean(curve_change, axis=1) * 180 / np.pi, color=(.5, .5, .5),
            linewidth=1, marker='o', markersize=4, zorder=2, label=modelname)
    # add fill between
    ...
    ax.plot(np.arange(curve_change_new.shape[0]), np.nanmean(curve_change_new, axis=1) * 180 / np.pi, color=(1, 0, 0),
            linewidth=1, marker='o', zorder=5, markersize=4, label="RWKV/rwkv-4-169m-pile")
    # add fill between
    ax.fill_between(np.arange(curve_change_new.shape[0]),
                    np.nanmean(curve_change_new, axis=1) * 180 / np.pi - np.nanstd(curve_change_new, axis=1) * 180 / np.pi,
                    np.nanmean(curve_change_new, axis=1) * 180 / np.pi + np.nanstd(curve_change_new, axis=1) * 180 / np.pi,
                    color=(1, 0, 0), alpha=.2)
    ...
    fig.show()
    fig.savefig(os.path.join(ANALYZE_DIR, f'curvature_{modelname}_vs_RWKA.pdf'), transparent=True)
    fig.savefig(os.path.join(ANALYZE_DIR, f'curvature_{modelname}_vs_RWKA.png'), transparent=True)
