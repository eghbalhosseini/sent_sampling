...
if __name__ == '__main__':
    ...
    modelclass = 'gpt2'
    modelname= 'gpt2'
    newmodelname="'EleutherAI_pythia-1.4b'"
    masked = False
    ...
    new_model_untrained = GPTNeoXForCausalLM.from_pretrained(
        "EleutherAI/pythia-1.4b",
        revision="step0")
    ...
    tokenized_new = [tokenizer_unt.tokenize(x) for x in sentences]
    # get ids
    indexed_tokens_new=[tokenizer_unt.convert_tokens_to_ids(x) for x in tokenized_new]
    ...
    mincons_model = scorer.IncrementalLMScorer('EleutherAI/pythia-1.4b', device='cuda')
    ...
    fig.savefig(os.path.join(ANALYZE_DIR, f'curvature_and_surpsial_{newmodelname}.pdf'), transparent=True)
    fig.savefig(os.path.join(ANALYZE_DIR, f'curvature_and_surprisal_{newmodelname}.png'), transparent=True)
