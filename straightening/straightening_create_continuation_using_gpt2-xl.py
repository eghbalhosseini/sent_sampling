...
import argparse
ANALYZE_DIR='/rdma/vast-rdma/vast/evlab/ehoseini/MyData/sent_sampling/analysis/straightening/generation/'
...
if __name__ == '__main__':
    ...
    path_continuation_dict.parent.mkdir(parents=True, exist_ok=True)
    save_sentence_path=Path(ANALYZE_DIR,f'sentences_{dataset}_cntx_{context_k}_cont_{continuation_k}.pkl')
    if save_sentence_path.exists():
        ...
        # make sure the directory exists
        save_sentence_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    for sent in sentences_:
        sent_tok_id = tokenizer(sent, return_tensors='pt')['input_ids'][0]
        assert(len(sent_tok_id)>(context_k+continuation_k))
        sent_token_all.append(tokenizer(sent, return_tensors='pt'))
        sentence_tokens.append(sent_tok_id.tolist())
    true_continuation=sentence_tokens
    ...
    # go to evey element in sent_token_ and and for each key get the first context_k tokens and add the to greedy_inputs
        greedy_inputs=[dict(input_ids=x['input_ids'][:1,:context_k],attention_mask=x['attention_mask'][:1,:context_k]) for x in sent_token_all]
        true_inputs=[dict(input_ids=x['input_ids'][:1,:(context_k+continuation_k)],attention_mask=x['attention_mask'][:1,:(context_k+continuation_k)]) for x in sent_token_all]
        greedy_tok_id=[]
        true_token_id=[]
        for i in tqdm(range(len(greedy_inputs))):
            ...
            true_token_id.append(true_inputs[i]['input_ids'].squeeze().tolist())

        # convert greedy_tok_id to list
        # make a dictionary for both greedy and true continuation
        continuations_dict=dict(greedy=greedy_tok_id,true=true_token_id,beam=[],sample=[],top_k=[],top_p=[])
