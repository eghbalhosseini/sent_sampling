...
if __name__ == '__main__':
    ...
    for layer_to_modify in layers_to_ablate:
        ...
        for name, param_src in ablated_state_dict.items():
            if f"h.{layer_to_modify}.mlp" in name:
    ...
    fig.savefig(os.path.join(ANALYZE_DIR, f'curvature_{modelname}_{dataset}_MLP_ablated.pdf'), transparent=True)
    fig.savefig(os.path.join(ANALYZE_DIR, f'curvature_{modelname}_{dataset}_MLP_ablated.png'), transparent=True)
