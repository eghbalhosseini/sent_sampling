...
# show the legend
    ...
    fig.savefig(os.path.join(ANALYZE_DIR, f'curvature_change_{basemodel_}_models_{dataset}_masked_{masked}.eps'),dpi=200,format='eps',bbox_inches='tight',transparent=True)
...
#%%
    ...
    #fig.show()
    basemodel_ = basemodel.replace('/', '_')
    fig.savefig(os.path.join(ANALYZE_DIR, f'curvature_change_{basemodel_}_models_{dataset}_masked_{masked}.eps'),
                dpi=200, format='eps', bbox_inches='tight', transparent=True)
    #%%
    fig = plt.figure(figsize=(8, 11), dpi=200, frameon=False)
    pap_ratio = 8 / 11
