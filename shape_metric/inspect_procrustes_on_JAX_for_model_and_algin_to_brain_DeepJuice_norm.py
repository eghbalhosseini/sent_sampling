...
if __name__ == '__main__':
    ...
    file=Path(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/shape_metric_jax_highres_vision_{group}_{method}_{adjust_mode}_{tol}_{n_init}_norm.pkl')# load file
    ...
    A = jax.device_put(jnp.array(X_bar_model, dtype=jnp.float64), device=jax.devices('gpu')[0])
    A_T = A.T
    ...
    for idx in tqdm(range(len(x_model))):
        x = x_model[idx]
        A_align = jnp.dot(x, jax_align(x, X_bar_model))
        aligned_Xbar_model.append(A_align)
        ATA_align = jnp.dot(A_align.T, A_align)
        trace_ATA_align = jnp.trace(ATA_align)
        print("Trace of A^T_align * A_align:", trace_ATA_align)
    ...
    for idx in tqdm(range(len(x_sub_fmri))):
        x=x_sub_fmri[idx]
        A_align=jnp.dot(x, jax_align(x, X_bar_model))
        aligned_Xbar_sub.append(A_align)
        ATA_align = jnp.dot(A_align.T, A_align)
        trace_ATA_align = jnp.trace(ATA_align)
        print("Trace of A^T_align * A_align:", trace_ATA_align)
    ...
    # Stack the differences into a single JAX array
    X_diff_model = jnp.stack(X_diff_model)
    # Compute the vector norms along the specified dimension
    X_var_model = jnp.linalg.norm(X_diff_model, ord=2, axis=-1)

    X_diff_sub = [x - X_bar_model for x in aligned_Xbar_sub]
    X_diff_sub = jnp.stack(X_diff_sub)
    X_var_sub = jnp.linalg.norm(X_diff_sub, ord=2, axis=-1)



    x = jnp.mean(X_var_model,axis=0)
    y= jnp.mean(X_var_sub,axis=0)
    ...
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_bar_model_distance_to_models_and_subjects_jax_{method}_{adjust_mode}_{tol}_{n_init}_{max_iter}_norm.png',dpi=300)
    # save eps
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_bar_model_distance_to_models_and_subjects_jax_{method}_{adjust_mode}_{tol}_{n_init}_{max_iter}_norm.eps',format='eps')
#%%
    ...
    for id_model in range(6):
        x_var_model = X_var_model[id_model]
        y = jnp.mean(X_var_sub,axis=0)
        ax = axs[id_model]
    ...
    fig.show()
    fig.savefig(
        f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_model_distance_to_x_bar_models_and_subjects_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_norm.png',
        dpi=300)
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_model_distance_to_x_bar_models_and_subjects_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_norm.eps')
    #%%
    #%% plot the distance between models and subjects#%%
    # do it for individuaal models
    ...
    fig.show()
    # save figure
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_model_pca_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_norm.png',dpi=300)
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_model_pca_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_norm.eps',format='eps')
    ...
    new_folder_name=f'shape_metric_jax_vision_{method}_{adjust_mode}_{tol}_{n_init}_{max_iter}_norm_image_order_1st_pc'
    ...
    x = X_var_model.mean(axis=0)[key_samples_indices]
    #x = pc1_scores[key_samples_indices]
    y = X_var_sub.mean(axis=0)[key_samples_indices]
    ...
    fig.show()
    fig.savefig(
        f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_bar_model_distance_to_models_and_subjects_color_by_PC_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_norm.png',
        dpi=300)
    # save eps
    fig.savefig(
        f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_bar_model_distance_to_models_and_subjects_color_by_PC_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_norm.eps',
        format='eps')
    ...
    axs = axs.ravel()
    for id_model in range(6):
        x_var_model = X_var_model[id_model][key_samples_indices]
        y = X_var_sub.mean(axis=0)[key_samples_indices]
        ax = axs[id_model]
    ...
    feature_map_min = [torch.from_numpy(np.array(x[min_var_idx, :])).to(torch.float32) for x in feature_map_all]
    feature_map_max = [torch.from_numpy(np.array(x[max_var_idx, :])).to(torch.float32) for x in feature_map_all]
    feature_map_rand = [torch.from_numpy(np.array(x[rand_var_idx, :])).to(torch.float32)for x in feature_map_all]
    ...
    score_all = get_cross_validated_benchmarking_results(benchmark=benchmark_, feature_extractor=[torch.from_numpy(np.array(x)).to(torch.float32) for x in feature_map_all],
                                                         extract_mode=extract_mode)
