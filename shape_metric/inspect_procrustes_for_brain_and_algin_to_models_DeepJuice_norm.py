...
if __name__ == '__main__':
    ...
    file = Path(
        f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/shape_metric_highres_vision_subjects_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm.pkl')
    ...
    X_bar_sub=results_dict['X_bar_model_final']
    torch.trace(torch.mm(X_bar_sub.T,X_bar_sub))
    X_bar_sub=normalize(X_bar_sub)
    ...
    for idx in tqdm(range(len(x_model))):
        x=x_model[idx]
        aligned_Xbar_model.append(x @ pt_align(x, X_bar_sub, group="orth",svd_solver=svd_solver))

    aligned_Xbar_sub=[]
    for idx in tqdm(range(len(x_sub_fmri))):
        x=x_sub_fmri[idx]
        aligned_Xbar_sub.append(x @ pt_align(x, X_bar_sub, group="orth",svd_solver=svd_solver))
    ...
    ax.scatter(x, y, color=[.5,.5,1], edgecolor='black', s=50)
    ...
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_bar_subject_distance_to_models_and_subjects_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm.png',dpi=300)
    # save eps
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_bar_subject_distance_to_models_and_subjects_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm.eps',format='eps')
#%%
    ...
    for id_model in range(6):
        ...
        ax.scatter(x_var_model, y, color=[.5,.5,1], edgecolor='black',
                   s=50)
    ...
    fig.show()
    fig.savefig(
        f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_model_distance_to_x_bar_subjects_and_subjects_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm.png',
        dpi=300)
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_model_distance_to_x_bar_subjects_and_subjects_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm.eps')
    #%%
    #%% plot the distance between models and subjects#%%
    # do it for individuaal models
    ...
    X_standardized = X_var_sub.T.cpu().numpy()
    ...
    colors = plt.cm.Blues(np.linspace(0, 1, len(key_samples_indices)))
    ...
    fig.show()
    # save figure
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_sub_pca_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm.png',dpi=300)
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_syb_pca_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm.eps',format='eps')
    ...
    new_folder_name=f'shape_metric_subject_vision_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm_image_order_1st_pc'
    ...
    # Display the plot
    fig.show()
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_bar_sub_distance_to_models_and_subjects_color_by_PC_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm.png',dpi=300)
    # save eps
    fig.savefig(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_bar_sub_distance_to_models_and_subjects_color_by_PC_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm.eps',format='eps')
    ...
    for region in all_regions:
        ...
        fig.show()
        fig.savefig(
            f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_var_brain_fit_by_brain_PC1_{region}_{grp}_{method}_{svd_solver}_{adjust_mode}_{tolerance}_{n_init}_{steps}_norm.png',
            dpi=300)
        # save eps
        fig.savefig(
