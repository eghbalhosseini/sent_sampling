...
if __name__ == '__main__':
    ...
    for k in range(10):
        X_bar, aligned_Xs_model = pt_frechet_mean(X_pad, group=grp, method=method, return_aligned_Xs=True,
                                                      max_iter=steps,
                                                      verbose=verbose, tol=tolerance)

        X_diff = [X - X_bar for X in aligned_Xs_model]
    ...
    X_bar_sub_final=torch.stack(aligned_Xbar_sub).mean(dim=0)
    ...
    data_for_plot={'X_var_all_final':X_var_all_final,
                   ...
                   'X_var_sub':X_var_sub}
    fig.savefig(
        f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_bar_model_distance_to_models_and_subjects.png',
        dpi=300)
    # save eps
    fig.savefig(
        f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/DeepJuice/figures/x_bar_model_distance_to_models_and_subjects.eps',
        format='eps')
    # save the data
    ...
    #%% plot the scatter between x_bar_model and x_bar_subjects
    ...
    aligned_X_bar_models = [x @ pt_align(x, X_bar_sub_final, group="orth") for x in X_pad]
    ...
    y = X_var_sub.mean(dim=0).cpu().numpy()
    fig, ax = plt.subplots(figsize=(8, 8))
    ax.scatter(x, y, color=[.3, .3, .3], edgecolor='black', s=50)
    # Plot the identity line
    min_val = min(min(x), min(y)) - 10
    max_val = max(max(x), max(y)) + 10
    ax.plot([min_val, max_val], [min_val, max_val], 'k-', linewidth=3)
