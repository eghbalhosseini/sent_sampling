...
if __name__ == '__main__':
    ...
    tolerance = 1e-5
    verbose = True
    save_path = Path(f'{act_dir}/multi_shape_distance_individual_DsParametric_{grp}_{adjust_mode}_{method}_pre_pca_{pre_pca}.pkl')
...
# do a pca on x_align_min and then transform x_align_max
    ...
    sns.histplot(data=df, y="y", hue="labels", palette=color_palette, ax=g.ax_marg_y, legend=False, binwidth=20,
                 element="step", fill=False)
    g.savefig(os.path.join(act_dir, f'DsParametricfMRI_Align_max_to_min_{grp}_{adjust_mode}_{method}.png'))
    # save eps
    g.savefig(os.path.join(act_dir, f'DsParametricfMRI_Align_max_to_min_{grp}_{adjust_mode}_{method}.eps'),
              format='eps')
    ...
    sns.histplot(data=df, y="y", hue="labels", palette=color_palette, ax=g.ax_marg_y, legend=False, binwidth=20,
                 element="step", fill=False)
    g.savefig(os.path.join(act_dir, f'DsParametricfMRI_Align_min_to_max_{grp}_{adjust_mode}_{method}.png'))
    ...
    sns.histplot(data=df, y="y", hue="labels", palette=color_palette, ax=g.ax_marg_y, legend=False, binwidth=20,
                 element="step", fill=False)
    g.savefig(os.path.join(act_dir, f'DsParametricfMRI__max_min_X_var_{grp}_{adjust_mode}_{method}.png'))
    # save eps
    g.savefig(os.path.join(act_dir, f'DsParametricfMRI_Align_max_to_min_{grp}_{adjust_mode}_{method}.eps'),
              format='eps')
    ...
    from scipy.io import savemat
    save_path = Path(f'{act_dir}/DsParametricfMRI_max_min_X_var_{grp}_{adjust_mode}_{method}.mat')
    # # make sure parent exist
    save_path.parent.mkdir(parents=True, exist_ok=True)
    # save as a mat file
    savemat(save_path, {'x': x_pca, 'labels': labels, 'image_id': sorted_image_ids, 'sent': sent_})