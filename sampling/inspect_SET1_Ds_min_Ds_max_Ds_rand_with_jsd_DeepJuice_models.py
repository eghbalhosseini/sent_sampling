...
if __name__ == '__main__':
    ...
    optim_id_min = f'coordinate_ascent_eh-obj=2-D_s_jsd-n_iter=500-n_samples={n_samples}-n_init=1-low_dim=False-pca_var=0.9-pca_type=sklearn-run_gpu=True'
    optim_id_max = f'coordinate_ascent_eh-obj=D_s_jsd-n_iter=500-n_samples={n_samples}-n_init=1-low_dim=False-pca_var=0.9-pca_type=sklearn-run_gpu=True'
    ...
    #                       f"{optim_obj.extractor_obj.identifier}_XY_corr_list-low_res=False-low_dim=False-pca_type={optim_obj.pca_type}-pca_var={optim_obj.pca_var}.pkl")
    # optim_obj.XY_corr_list = torch.load(xy_dir,map_location=torch.device('cpu'))

    # optim_no_jsd
    optim_id_min_no_jsd = f'coordinate_ascent_eh-obj=2-D_s-n_iter=500-n_samples={n_samples}-n_init=1-low_dim=False-pca_var=0.9-pca_type=sklearn-run_gpu=True'
    optim_obj_no_jsd = optim_pool[optim_id_min_no_jsd]()
    optim_obj_no_jsd.N_S = 1000
    optim_obj_no_jsd.extract_type = 'activation'
    optim_obj_no_jsd.activations = activations_list
    optim_obj_no_jsd.extractor_obj = ext_obj
    optim_obj_no_jsd.early_stopping = False
    optim_obj_no_jsd.precompute_corr_rdm_on_gpu(low_resolution=False, cpu_dump=False, preload=True,
                                         save_results=False)
    ...
    ds_min_path=f'{RESULTS_DIR}/results_{extract_short_hand}_{optim_short_hand_min}_{extract_mode}_{suffix}.pkl'
    ...
    ds_max_path = f'{RESULTS_DIR}/results_{extract_short_hand}_{optim_short_hand_max}_{extract_mode}_{suffix}.pkl'
    ...
    ds_rand_path = f'{RESULTS_DIR}/results_{extract_short_hand}_{optim_short_rand}_{extract_mode}_{suffix}.pkl'
    ...
    if Path(ds_rand_path).exists():
        ...
        for k in tqdm(enumerate(range(1000))):
            sent_random = list(np.random.choice(optim_obj.N_S, optim_obj.N_s,replace=False))
        ...
        optim_file = Path(RESULTS_DIR, f"results_{extract_short_hand}_{optim_short_rand}_{extract_mode}_{suffix}.pkl")
    ...
    save_path = Path(ANALYZE_DIR, 'DsParametric', f'ds_data_Parametric_deepJuice_n_{n_samples}_{extract_mode}_{suffix}.pkl')
    save_path.parent.mkdir(parents=True, exist_ok=True)
    with open(save_path.__str__(), 'wb') as f:
        pickle.dump(figure_3_data, f)
    save_path = Path(ANALYZE_DIR, 'DsParametric', f'RDM_max_dict_parametric_deepJuice_n_{n_samples}_{extract_mode}_{suffix}.pkl')
    with open(save_path.__str__(), 'wb') as f:
        pickle.dump(RDM_max_dict, f)
    save_path = Path(ANALYZE_DIR, 'DsParametric', f'RDM_min_dict_parametric_deepJuice_n_{n_samples}_{extract_mode}_{suffix}.pkl')
    with open(save_path.__str__(), 'wb') as f:
        pickle.dump(RDM_min_dict, f)
    save_path = Path(ANALYZE_DIR, 'DsParametric', f'RDM_rand_dict_parametric_deepJuice_n_{n_samples}_{extract_mode}_{suffix}.pkl')
    with open(save_path.__str__(), 'wb') as f:
        pickle.dump(RDM_rand_dict, f)
    save_path = Path(ANALYZE_DIR, 'DsParametric', f'RDM_all_dict_parametric_deepJuice_n_{n_samples}_{extract_mode}_{suffix}.pkl')
    with open(save_path.__str__(), 'wb') as f:
    ...
    for kk in tqdm(range(1000)):
        ...
        _, _, jsd_min = optim_obj.gpu_object_function_ds_plus_jsd(ds_min_loc, debug=True)
        _, _, jsd_rand = optim_obj.gpu_object_function_ds_plus_jsd(ds_rand_loc, debug=True)
        _, _, jsd_max = optim_obj.gpu_object_function_ds_plus_jsd(ds_max_loc, debug=True)
