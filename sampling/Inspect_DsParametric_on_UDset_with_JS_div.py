...
if __name__ == '__main__':
    ...
    optimizer_id_max = f"coordinate_ascent_eh-obj=D_s_jsd-n_iter=50-n_samples=225-n_init=1-low_dim=False-pca_var=0.9-pca_type=pytorch-run_gpu=True"

    [ext_id, opt_id_max] = make_shorthand(extract_id, optimizer_id_max)
    optim_file = os.path.join(RESULTS_DIR,f'res_{ext_id}_{opt_id_max}_jsd_thr_{jsd_threshold}_mult_{jsd_muliplier}_bins_{bins}_norm.pkl')
    ...
    # load optim_max
    optimizer_id_min = f"coordinate_ascent_eh-obj=2-D_s_jsd-n_iter=50-n_samples=225-n_init=1-low_dim=False-pca_var=0.9-pca_type=pytorch-run_gpu=True"
    [ext_id, opt_id_min] = make_shorthand(extract_id, optimizer_id_min)
    optim_file = os.path.join(RESULTS_DIR,f'res_{ext_id}_{opt_id_min}_jsd_thr_{jsd_threshold}_mult_{jsd_muliplier}_bins_{bins}_norm.pkl')
    if os.path.exists(optim_file):
        modification_time = os.path.getmtime(optim_file)
        modification_date = time.ctime(modification_time)
        print(f"The file was last modified on: {modification_date}")
...
#%%
    ...
    #%
    ds_rand_loc = np.random.choice(optimizer_obj.N_S, optimizer_obj.N_s, replace=False)
    d_s_min, RDM_min = optimizer_obj.gpu_object_function_debug(ds_min_loc)
    [ds_jsd_min, _, jsd_min]=optimizer_obj.gpu_object_function_ds_jsd(ds_min_loc, debug=True)
    d_s_rand, RDM_rand = optimizer_obj.gpu_object_function_debug(ds_rand_loc)
    [ds_jsd_rand, _, jsd_rand]=optimizer_obj.gpu_object_function_ds_jsd(ds_rand_loc, debug=True)
    d_s_max, RDM_max = optimizer_obj.gpu_object_function_debug(ds_max_loc)
    [ds_jsd_max, _, jsd_max]=optimizer_obj.gpu_object_function_ds_jsd(ds_max_loc, debug=True)
    ...
    save_loc = Path(save_path.__str__(), f'ds_{ext_sh}_{optim_sh}_jsd_thr_{optimizer_obj.jsd_threshold}_mult_{optimizer_obj.jsd_muliplier}_bins_{optimizer_obj.bins}_norm.png')
    fig.savefig(save_loc.__str__(), format='png', metadata=None, bbox_inches=None, pad_inches=0.1, dpi=350,
                facecolor='auto',
                edgecolor='auto', backend=None)
    save_loc = Path(save_path.__str__(), f'ds_{ext_sh}_{optim_sh}_jsd_thr_{optimizer_obj.jsd_threshold}_mult_{optimizer_obj.jsd_muliplier}_bins_{optimizer_obj.bins}_norm.eps')
    ...
    plt.tight_layout()
    fig.show()
    ax_title = f'S_feat,{ext_sh}_{optim_sh}_jsd_thr_{optimizer_obj.jsd_threshold}_mult_{optimizer_obj.jsd_muliplier}_bins_{optimizer_obj.bins}_norm'
    ...
    ax_title = f'S_rdm_{ext_sh}_{optim_sh}_jsd_thr_{optimizer_obj.jsd_threshold}_mult_{optimizer_obj.jsd_muliplier}_bins_{optimizer_obj.bins}_norm'
    # save the figure
    save_path = Path(ANALYZE_DIR)
    save_loc = Path(save_path.__str__(), f'{ax_title}.png')
    fig.savefig(save_loc.__str__(), format='png', metadata=None, bbox_inches=None, pad_inches=0.1, dpi=350)
    ...
    ax_title = f'S_align_{ext_sh}_{optim_sh}_jsd_thr_{optimizer_obj.jsd_threshold}_mult_{optimizer_obj.jsd_muliplier}_bins_{optimizer_obj.bins}_norm'
    # save the figure
    save_path = Path(ANALYZE_DIR)
    save_loc = Path(save_path.__str__(), f'{ax_title}.png')
    fig.savefig(save_loc.__str__(), format='png', metadata=None, bbox_inches=None, pad_inches=0.1, dpi=350)
...
#% compute the actuall difference between sentence similarities
    ...
    for idx in range(len(x_min_)):
        ...
        #rand_to_rand_coeff = np.asarray(rand_to_rand_coeff)
        ...
        # # randomly select max_to_rand_coeff.shae from rand_coef_vec
        # rand_coef_vec_sample = np.random.choice(rand_coef_vec, size=max_to_rand_coeff.shape[0], replace=False)
        # # check if max_to_rand_coeff and rand_coeff come from same distribution
        # # print p-value up to 4 decimal points
        # mw_stat, mw_p = mannwhitneyu(max_to_rand_coeff, rand_coef_vec_sample)
        ...
        # mw_stat, mw_p = mannwhitneyu(min_to_rand_coeff, rand_coef_vec_sample)
        # print(f"{model_names[idx]} Min to Rand Mann-Whitney U Test: statistic={mw_stat}, p-value={mw_p:.8f}")
        # # do min to max
        # mw_stat, mw_p = mannwhitneyu(min_to_rand_coeff, max_to_rand_coeff)
        # print(f"{model_names[idx]} Min to Max Mann-Whitney U Test: statistic={mw_stat}, p-value={mw_p:.8f}")
    ...
    # save figure
    ax_title=f'S_jsd_{ext_sh}_{optim_sh}_jsd_thr_{optimizer_obj.jsd_threshold}_mult_{optimizer_obj.jsd_muliplier}_bins_{optimizer_obj.bins}_norm'
    fig.show()
    fig.savefig(
        os.path.join(ANALYZE_DIR,f'{ax_title}.png'), format='png', metadata=None, bbox_inches=None, pad_inches=0.1, dpi=350)
    # save as eps
    fig.savefig(os.path.join(ANALYZE_DIR,f'{ax_title}.eps'))
    ...
    #     #print(f"{model_names[idx]} Mann-Whitney U Test: statistic={mw_stat}, p-value={mw_p}")
    #
    # ax_title = f'jsd_{ext_sh}_{optim_sh}_jsd_thr_{optimizer_obj.jsd_threshold}_mult_{optimizer_obj.jsd_muliplier}_bins_{optimizer_obj.bins}_norm'
    # # save the figure
    # save_path = Path(ANALYZE_DIR)
    # save_loc = Path(save_path.__str__(), f'{ax_title}.png')
    # fig.savefig(save_loc.__str__(), format='png', metadata=None, bbox_inches=None, pad_inches=0.1, dpi=350)
