...
#%%
    ...
    if Path(ds_rand_path).exists():
        with open(ds_rand_path.__str__(), 'rb') as f:
    ...
    ax_title = f'ds_data,{ext_sh}_{optim_sh}_kl_div_thr_{kl_threshold}_mult_{kl_muliplier}_bins_{bins}'
    ...
    pap_ratio = 8 / 11
    for kk in range(len(model_names)):
